...
const ReactNativePickerModule = ({
    ...
    const [selectedLabel, setSelectedIndex] = useState(0)



    const setDefaultSelectedKey = () => setSelectedKey(
        value || (typeof items[0] === "object" ? items[0].key : items[0])
    )
                ...
                <Picker
                    ...
                    {items.map((item, index) => {
                        if (item.hasOwnProperty("key") && item.hasOwnProperty("label")) {
                            return (
                                <Picker.Item
                                    color={value === item.key ? selectedColor : undefined}
                                    key={"item-" + index}
                ...
                <TouchableOpacity
                    ...
                    <Text
                        style={[
                            ...
                            selectedKey === value ? confirmButtonDisabledTextStyle : confirmButtonEnabledTextStyle,
                        ]}>
                        {confirmButton}
                    </Text>
                </TouchableOpacity>
